...
import { FormType, ISubject, ISubjectFormBody } from '../../interfaces';
...
export function SubjectForm() {
  ...
  useEffect(() => {
    ...
    const getSubjectDetail = async () => {
      const response: IBodyResponse<ISubject> = await subjectService._getDetail(
        selectedSubject?.id ?? '',
      );
      if (response.success) {
        form.reset({
  ...
  const onSubmit = async (data: ISubjectFormBody) => {
    ...
    try {
      ...
      if (
        response?.errors &&
        response.errors[0]?.errorKey === 'subject.error.subject.existed'
      ) {
  ...
  return (
    ...
    >
      <div className="flex flex-col items-start justify-end gap-2.5 pb-2">
        ...
        <div className="text-[#E9034E]">{t(`subjects.form.${formType}_message`)}</div>
      </div>
      <div
        className={cn(
