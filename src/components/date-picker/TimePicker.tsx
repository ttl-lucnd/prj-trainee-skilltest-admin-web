...
import { DateTimePickerProps, TimeOption } from './interfaces';
import { AM_VALUE, buildTime, HOURS_IN_DAY, HOURS_IN_HALF_DAY, PM_VALUE, SECONDS_IN_MINUTE } from './helpers';
...
export function TimePicker({
  ...
  useEffect(() => {
    onChange(
      buildTime({ use12HourFormat, value, formatStr, hour, minute, second, ampm }),
    );
  }, [hour, minute, second, ampm, formatStr, use12HourFormat]);

  const _hourIn24h = useMemo(() => {
    return use12HourFormat ? (hour % HOURS_IN_HALF_DAY) + ampm * HOURS_IN_HALF_DAY : hour;
  }, [value, use12HourFormat, ampm]);

  const hours: TimeOption[] = useMemo(
    () =>
      Array.from({ length: use12HourFormat ? HOURS_IN_HALF_DAY : HOURS_IN_DAY }, (_, i) => {
        let disabled = false;
        const hourValue = use12HourFormat && i === 0 ? HOURS_IN_HALF_DAY : i;
        const hourInDay = use12HourFormat ? i + ampm * HOURS_IN_HALF_DAY : i;
        const hDate = setHours(value, hourInDay);
  ...
  const seconds: TimeOption[] = useMemo(() => {
    ...
    return Array.from({ length: SECONDS_IN_MINUTE }, (_, i) => {
      let disabled = false;
  ...
  const ampmOptions = useMemo(() => {
    ...
    return [
      ...
      let disabled = false;
      const start = addHours(startD, v.value * HOURS_IN_HALF_DAY);
      const end = subHours(endD, (1 - v.value) * HOURS_IN_HALF_DAY);
  ...
  const onAmpmChange = useCallback(
    (v: TimeOption) => {
      if (min) {
        ...
        if (newTime < min) {
          const minH = min.getHours() % HOURS_IN_HALF_DAY;
          setHour(minH === 0 ? HOURS_IN_HALF_DAY : minH);
          setMinute(min.getMinutes());
          setSecond(min.getSeconds());
        }
      }
      if (max) {
        ...
        if (newTime > max) {
          const maxH = max.getHours() % HOURS_IN_HALF_DAY;
          setHour(maxH === 0 ? HOURS_IN_HALF_DAY : maxH);
          setMinute(max.getMinutes());
          setSecond(max.getSeconds());
        }
      }
  ...
  const display = useMemo(() => {
    const timeElements = ['hour', 'minute', 'second']
      .filter((el) => !timePicker || timePicker[el as keyof typeof timePicker])
      .map((el) => {
        if (el === 'hour') return use12HourFormat ? 'hh' : 'HH';
        return el === 'minute' ? 'mm' : 'ss';
      });

    const formatString = timeElements.join(':') + (use12HourFormat ? ' a' : '');
...
const TimeItem = ({
  ...
  return (
    <Button
      ...
      disabled={disabled}
    >
      <div className="w-4">{selected && <CheckIcon className="my-auto size-4" data-testid="check-icon" />}</div>
